...
- Task IDs are UUIDs
- Profile usernames should not include the `@` symbol
- Some endpoints return task IDs - use the Tasks endpoint to check status and retrieve results
- Instagram and TikTok search endpoints are currently disabled
- The `/metrics` endpoint is excluded from the global API prefix and returns Prometheus format
...
- Review the project structure and codebase for implementation details
